...
The Data Frame is the most important structured API in Spark and represents a fundamental structure for working with data.
Here is a detailed explanation of DataFrames based on the source:
Structure and Definition
• API Type: DataFrames are the most common structured API and are built upon low-level APIs, such as RDD.
• Appearance: A DataFrame looks like a simple table, consisting of rows and columns.
• Schema: A schema is associated with a data frame. This schema means that each column will have its own metadata.
Storage and Parallelism
• Partitions: The data within DataFrames are stored in partitions.
• Parallel Execution: The use of partitions is the main reason why Spark prefers keeping data in partitions. When a transformation is applied and an action is called on a DataFrame:
    ◦ The data partitions are shared across the cluster.
    ◦ Each core can work on each partition of data with different tasks.
    ◦ All the tasks can execute in parallel.
Immutability
DataFrames are immutable, which is a key concept:
• Meaning: Immutability simply means that once a data frame is created, you can't change anything about the data frame.
• Creating New DataFrames: While you cannot modify an existing DataFrame, you can create a new data frame out of the data frame.
• Transformation Example: If you read employee data into a first DataFrame:
    ◦ You can create a second DataFrame by filtering the data (e.g., selecting data where the salary is greater than 10,000).
    ◦ You can then create a third DataFrame from the second one by selecting only specific columns (like name, Department ID, and salary).
• Cascading: Another method is cascading, where you can cascade more than one transformation commands from one DataFrame to create a new DataFrame.

--------------------------------------------------------------------------------
A DataFrame can be thought of as a spreadsheet with a defined structure (the schema), but where every change you make doesn't erase the original sheet; instead, it generates a perfectly new, optimized version for Spark to process in parallel.


** data frames is the most  structured api  represent table 
-  table represent rows and columns 
- dataframe have schema , which is the metadata of the columns 
- data in dataframe  are in partitions 
-dataframe are immutable 

* one task takes one core so i excutor have 2 cores  where parallel excuitor have 4 cores means 4 tasks 

 structured api are of two types 
